...
for r = 1:runs
    ...
    % average fft and store magnitude (real-valued power) for each run
    dataL(r, :)  = abs(avgL  / nFFT);
    dataNL(r, :) = abs(avgNL / nFFT);
end
...
binNames = arrayfun(@(k) sprintf('bin_%d', k), 0:N-1, 'UniformOutput', false);
