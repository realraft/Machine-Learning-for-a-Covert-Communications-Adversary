...
def oversample_minority(X_train, y_train, random_state=42):
    ...
    MLPClassifier has no class_weight argument, so we handle imbalance here.
    """
    classes, counts = np.unique(y_train, return_counts=True)
    if counts.max() == counts.min():
        return X_train, y_train          # already balanced
    ...
    # Shuffle so minority copies aren't all at the end
...
def main(args):
    ...
    # ---- Feature scaling (fit on balanced training set) ----
    ...
    #   tol=1e-5 prevents the default loose tolerance from declaring
    #   convergence after only ~20 iterations.
    model = MLPClassifier(
